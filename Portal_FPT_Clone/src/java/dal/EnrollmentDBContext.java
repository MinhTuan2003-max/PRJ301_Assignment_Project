...
public class EnrollmentDBContext extends DBContext<Enrollment> {
    ...
    public ArrayList<Enrollment> getByGroupAndCourse(int group_id, int course_id, int semester_id, int department_id) {
        ...
        try {
            String sql = "SELECT s.[student_id]\n"
                    ...
                    + "  WHERE g.[group_id] = ? AND g.[course_id] = ? AND g.semester_id = ? AND c.[department_id] = ?\n"
                    + "  ORDER BY s.student_ID";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, group_id);
            stm.setInt(2, course_id);
            stm.setInt(3, semester_id);
            stm.setInt(4, department_id);
